...
const Login = () => {
  ...
  const [revieveErr, setRecieveErr] = useState("");
  ...
  const submitFrom = async e => {
    ...
    if (formData.email.length < 6) {
      ...
      setLoading(true)
      // console.log(submitFormStates)

      // alert(import.meta.env.API)
      // setTimeout(()=>{
      //   setSubmitFormStates({...submitFormStates, ["loading"]:false})
      //   console.log(submitFormStates)
      // }, 5000)


      try {
        const request = await fetch('https://ghaznaprogrammar.com/api/user/login', {
          method: "POST",
